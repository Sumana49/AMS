...
$.fn.dataTableExt.oPagination.input = {
	"fnInit": function ( oSettings, nPaging, fnCallbackDraw )
	{
		...
		nFirst.style.visibility='hidden';
		...
		$(nInput).keyup( function (e) {
			...
			if ( this.value === "" || this.value.match(/[^0-9]/) || oSettings.fnRecordsDisplay() == 0 )
			{
	...
	"fnUpdate": function ( oSettings, fnCallbackDraw )
	{
		...
		if (iCurrentPage == 1) {
			$('#'+oSettings.sTableId+'_previous').css('pointer-events', 'none');
		} else {
			$('#'+oSettings.sTableId+'_previous').css('pointer-events', 'auto');
		}
		if (iCurrentPage == iPages) {
			$('#'+oSettings.sTableId+'_next').css('pointer-events', 'none');
		} else {
			$('#'+oSettings.sTableId+'_next').css('pointer-events', 'auto');
		}
